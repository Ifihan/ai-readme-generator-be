...
async def get_readme_history_entry(
    entry_id: str, username: str
) -> Optional[ReadmeHistoryEntry]:
...
async def clear_all_readme_history(username: str) -> int:
    """Delete all README history entries for a user."""
    db = get_database()
    collection = db.readme_history
...
async def get_user_readme_stats(username: str) -> Dict[str, Any]:
    ...
    recent_count = await collection.count_documents(
        {"username": username, "created_at": {"$gte": thirty_days_ago}}
    )
